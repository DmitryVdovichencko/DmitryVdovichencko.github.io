---
title: "Руководство по авторизации с Passport JS" 
date: "2020-12-20"
cover: ./assets/passport-js-course.png
embeddedImagesLocal:
- ./assets/auth-types.png
- ./assets/passport-js-course.png
tags: ["js","ru"]
---
import { getImage, GatsbyImage } from 'gatsby-plugin-image';
import PostImage from '../../../components/ui/PostImage';

# Руководство по авторизации с помощью Passport JS

<PostImage  alt="passport-js-course" imageData={props.localImages[1]} />

В этой статье, мы разберемся как работает JWT стратегия авторизации Passport JS. Эта стратегия - простое и безопасное решение для небольших проектов, работающих на связке Node/Express + фронтенд на любом из популярных JS фреймворков/библиотек (Angular, Vue, React).

Чтобы определится с оптимальным решением, мы пробежимся по доступным механизмам авторизации.  разберемся как они работают, и как их реализовать в приложении (за исключением протокола OAuth, который здесь рассматриваться не будет). 

Пост получился довольно детальным. Если вы уже знакомы с какой-либо темой смело переходите к тому, что вас интересует:

- [Авторизация с помощью сессий]()
- [Авторизация с помощью JWT]()

Посмотреть на примеры приложений с реализациями способов авторизации можно здесь:
...
## Выбираем механизма авторизации

<PostImage  alt="auth-types" imageData={props.localImages[0]} />

Основные варианты дрступных механизмов авторизации:

- __Session - Авторизация на основе сессии__ - Использует Cookies браузера совместно с сессиями ("Sessions") на бэкенде.

- __JWT Авторизация__ - JSON Web token (JWT) записывается в браузере (обычно в localStorage). В JWT хранятся данные пользователя и они могут быть расшифрованы только с помощью секретного ключа, который хранится на сервере.
...
## Авторизация на основе сессии (Session Based Authentication)?

Если бы мы делали родословную методов авторизации, авторизация на основе сессии была бы самой старшей, но, определенно не устаревшей. Этот метод авторизации "server-side" - Express приложение и база данных работают вместе, чтобы сохранить текущий статус авторизации каждого посетителя.

Чтобы понять как работает авторизация на основе сессии, мы разберемся в следующих темах:
